...
export const MicroImprovementPage: React.FC<MicroImprovementPageProps> = ({ title, links, onNavigate }) => {
  ...
  const dashboardUrl = "https://www.kdocs.cn/l/cdyYuiCbYIBN?R=L1MvMw==&disableNoviceGuide=";
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-8 scroll-smooth">
        <div className="max-w-[1600px] mx-auto space-y-8">
            ...
            {links.map((link, index) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`group relative overflow-hidden transition-all duration-300 transform hover:-translate-y-1.5 hover:brightness-110 shadow-xl ${theme.shadow} ${theme.bg} flex flex-row items-center w-full h-[72px] px-4 rounded-[2rem] lg:flex-col lg:items-start lg:justify-between lg:w-1/3 lg:max-w-[400px] lg:h-[150px] lg:p-6 lg:rounded-[2rem]`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`font-light opacity-90 leading-tight ${theme.text} text-[10px] line-clamp-1 lg:text-sm lg:line-clamp-2`}>{link.description}</p>
                  </div>
          ...
          <div className="relative group min-h-[400px]">
            <div className={`relative bg-white border-[4px] border-white ring-1 ring-slate-200 shadow-2xl overflow-hidden rounded-2xl h-[650px] lg:rounded-[2.5rem] lg:h-[650px]`}>
              
              {/* iOS 兼容性补丁桥接 */}
              <SafariBridge url={dashboardUrl} title="微改善看板" onUnlocked={() => {}} />
